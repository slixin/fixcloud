...
app.controller('ctrlFixClient', ['$scope', '$http', '$timeout', '$ngBootbox', '$cookies', function($scope, $http, $timeout, $ngBootbox, $cookies) {
    ...
    $scope.$on('onSessionChangeEvent', function(event, msg) {
        var targetTabs = $.grep($scope.settingTabs, function(e) {
            return e.id == msg.id
        });
...
app.controller("ctrlSession", ['$scope', '$http', '$interval', '$ngBootbox', function($scope, $http, $interval, $ngBootbox) {
    ...
    var removeIgnoreTags = function(messageArray) {
    ...
    var buildGroupMessage = function(messageArray) {
        var groupHeadTag = null;
        var groupArray = [];
        var returnArray = [];

        var workingArray = messageArray.slice(0);
        var groupHeadTag = workingArray[0].split('=')[0].trim();

        jQuery.each(workingArray, function(i, val) {
            var tagname = val.split('=')[0].trim();
            if (i > 0 && tagname == groupHeadTag) {
                var gArray = buildNormalizedMessage(groupArray);
                returnArray.push(gArray);
                groupArray = [];
                groupArray.push(val);
            } else {
                groupArray.push(val)
    ...
    var buildNormalizedMessage = function(messageArray) {
        var workingArray = messageArray.slice(0);
        var repeatGroups = ["555", "552"];
        var returnArray = {};
        ...
        jQuery.each(workingArray, function(i, val) {
            if (val != undefined) {
                ...
                if (repeatGroups.indexOf(tagname) > -1) {
                    var groupMessage = workingArray.splice(i + 1, workingArray.length - i - 1);
                    returnArray[tagname] = { "value": tagvalue, "groups": [] };
                    var groupMessageArray = buildGroupMessage(groupMessage);
                    returnArray[tagname].groups = groupMessageArray;
                    return returnArray;
                } else {
                    returnArray[tagname] = tagvalue;
                }
            }
        });
    ...
    var normalizeMessage = function(message) {
        ...
        removeIgnoreTags(messageArray);
        messageArray.unshift("8=FIX."+$scope.session.version);
        normalized = buildNormalizedMessage(messageArray);
        ...
        $http.post('/fix/logon', {
            ...
            if (resp.data != null) {
                if (resp.data.status == "ok") {
                    ...
                    timer = $interval(getMessages, 2000);
